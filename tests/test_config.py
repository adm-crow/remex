...
from remex.core.config import load_config, save_config


def _write_toml(tmp_path: Path, content: str) -> Path:
    f = tmp_path / "remex.toml"
...
def test_cli_uses_config_db(mock_ingest, tmp_path):
    _write_toml(tmp_path, '[remex]\ndb = "./custom_db"\n')
    with patch("remex.core.config.Path") as mock_path_cls:
    ...
    with patch("remex.cli.load_config", return_value=load_config(tmp_path)):
        CliRunner().invoke(cli, ["ingest", str(tmp_path)])
...
def test_cli_flag_overrides_config(mock_ingest, tmp_path):
    """A CLI flag always wins over remex.toml."""
    _write_toml(tmp_path, '[remex]\ndb = "./config_db"\n')
    with patch("remex.cli.load_config", return_value=load_config(tmp_path)):
...
def test_save_config_replaces_remex_section(tmp_path):
    """Calling save_config twice must overwrite the [remex] section, not append."""
    save_config({"db": "./first"}, root=tmp_path)
    save_config({"db": "./second"}, root=tmp_path)
    content = (tmp_path / "remex.toml").read_text(encoding="utf-8")
    assert content.count("[remex]") == 1
